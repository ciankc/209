...
class RoomRF():
    def __init__(self):
        for room in rooms:
            self.model, self.train, self.test, self.train_labels, self.test_labels = self.run_train(room)
       
    def evaluate():
        model, train, test, train_labels, test_labels = self.model, self.train, self.test, self.train_labels, self.test_labels
    ...
    def run_infer(self, test):
        print("======================TEST")
        print(test)
        predictions = self.model.predict(test)
        probs = self.model.predict_proba(test)[:, 1]
        print("======================PREDICTIONS")
        print(predictions)
        print("======================PROBS")
        print(probs)
    ...
    def run_train(self, room):
        ...
        test = pd.DataFrame([1583383222], index=[0], columns=features)
