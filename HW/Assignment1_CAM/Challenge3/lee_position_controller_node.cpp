...
void LeePositionControllerNode::MultiDofJointTrajectoryCallback(
  ...
  if(eigen_reference.position_W.x() < -1.0 &&
     eigen_reference.position_W.y() > 1.0 &&
     eigen_reference.position_W.z() > 0.5)
  {
      Eigen::Vector3d desired_position(-2.0,2.0,1.0);
      double desired_yaw = 0.0;
      trajectory_msgs::MultiDOFJointTrajectory msg1;
      mav_msgs::msgMultiDofJointTrajectoryFromPositionYaw(desired_position, desired_yaw, &msg1);
      mav_msgs::eigenTrajectoryPointFromMsg(msg1.points.front(), &eigen_reference);
  }
